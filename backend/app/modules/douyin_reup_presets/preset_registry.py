from __future__ import annotations

from copy import deepcopy

from app.modules.douyin_reup.douyin_schema import DouyinReupSettings
from app.modules.douyin_reup_presets.preset_schema import DouyinReupPreset, DouyinReupPresetMode


def list_douyin_reup_presets() -> list[DouyinReupPreset]:
    return deepcopy(_PRESETS)


def get_douyin_reup_preset(preset_id: str) -> DouyinReupPreset | None:
    cleaned = str(preset_id).strip().lower()
    for preset in _PRESETS:
        if preset.id.value == cleaned:
            return preset.model_copy(deep=True)
    return None


def _settings(preset_id: DouyinReupPresetMode, preset_name: str, **updates) -> DouyinReupSettings:
    payload = {
        "enabled": True,
        "preset_id": preset_id.value,
        "preset_name": preset_name,
        **updates,
    }
    return DouyinReupSettings.model_validate(payload)


_PRESETS: list[DouyinReupPreset] = [
    DouyinReupPreset(
        id=DouyinReupPresetMode.safe_review,
        name="Safe Review",
        description="Use available subtitles first, fall back to ASR/OCR, then stop for subtitle review.",
        recommended_for=["mixed subtitle quality", "first run", "quality control"],
        not_recommended_for=["fully unattended render"],
        ui_badge="Default",
        is_default=True,
        settings=_settings(
            DouyinReupPresetMode.safe_review,
            "Safe Review",
            subtitle_source_priority=["sidecar_srt", "embedded_subtitle", "asr", "ocr_hardsub"],
            use_sidecar_srt=True,
            use_embedded_subtitle=True,
            use_asr_if_no_subtitle=True,
            use_ocr_if_asr_failed=True,
            use_ocr_if_no_subtitle=True,
            prefer_ocr_over_asr_when_text_visible=False,
            review_subtitles_before_render=True,
            auto_render_after_translation=False,
            auto_mark_low_quality_lines=True,
            enable_subtitle_rewrite_suggestions=True,
            auto_generate_rewrite_for_flagged_lines=False,
            auto_apply_safe_rewrites=False,
            add_overlay=False,
            overlay_mode="none",
            subtitle_cover_enabled=True,
            add_bgm=True,
            keep_original_audio=True,
            bgm_volume=0.16,
            original_audio_volume=0.85,
        ),
    ),
    DouyinReupPreset(
        id=DouyinReupPresetMode.fast_auto,
        name="Fast Auto",
        description="Process and render without subtitle review, using ASR first with OCR fallback.",
        recommended_for=["known-good inputs", "quick batch", "low manual review"],
        not_recommended_for=["new sources", "hardcoded Chinese subtitles only"],
        ui_badge="Fast",
        settings=_settings(
            DouyinReupPresetMode.fast_auto,
            "Fast Auto",
            subtitle_source_priority=["sidecar_srt", "embedded_subtitle", "asr", "ocr_hardsub"],
            use_sidecar_srt=True,
            use_embedded_subtitle=True,
            use_asr_if_no_subtitle=True,
            use_ocr_if_asr_failed=True,
            use_ocr_if_no_subtitle=True,
            asr_model_size="tiny",
            asr_vad_filter=True,
            ocr_sample_fps=2.0,
            ocr_min_confidence=0.35,
            review_subtitles_before_render=False,
            auto_render_after_translation=True,
            enable_subtitle_rewrite_suggestions=False,
            auto_generate_rewrite_for_flagged_lines=False,
            auto_apply_safe_rewrites=False,
            add_overlay=False,
            overlay_mode="none",
            subtitle_cover_enabled=True,
            add_bgm=True,
            keep_original_audio=True,
            bgm_volume=0.18,
            original_audio_volume=0.82,
        ),
    ),
    DouyinReupPreset(
        id=DouyinReupPresetMode.ocr_priority,
        name="OCR Priority",
        description="Prefer hardcoded subtitle OCR before ASR when Chinese text is visible.",
        recommended_for=["hardcoded Chinese subtitles", "low voice clarity", "music-heavy videos"],
        not_recommended_for=["no visible subtitles", "very small text"],
        ui_badge="OCR",
        settings=_settings(
            DouyinReupPresetMode.ocr_priority,
            "OCR Priority",
            subtitle_source_priority=["sidecar_srt", "embedded_subtitle", "ocr_hardsub", "asr"],
            use_sidecar_srt=True,
            use_embedded_subtitle=True,
            use_asr_if_no_subtitle=True,
            use_ocr_if_asr_failed=True,
            use_ocr_if_no_subtitle=True,
            prefer_ocr_over_asr_when_text_visible=True,
            ocr_provider="easyocr",
            ocr_sample_fps=3.0,
            ocr_min_confidence=0.35,
            review_subtitles_before_render=True,
            auto_render_after_translation=False,
            add_overlay=False,
            overlay_mode="none",
            subtitle_cover_enabled=True,
            add_bgm=True,
            keep_original_audio=True,
        ),
    ),
    DouyinReupPreset(
        id=DouyinReupPresetMode.voice_priority,
        name="Voice Priority",
        description="Use speech recognition as the primary fallback, then auto-render MP4 after translation.",
        recommended_for=["clear speech", "few visible captions", "speaker-led videos"],
        not_recommended_for=["music-only clips", "subtitle-only videos"],
        ui_badge="Voice",
        settings=_settings(
            DouyinReupPresetMode.voice_priority,
            "Voice Priority",
            subtitle_source_priority=["sidecar_srt", "embedded_subtitle", "asr", "ocr_hardsub"],
            use_sidecar_srt=True,
            use_embedded_subtitle=True,
            use_asr_if_no_subtitle=True,
            use_ocr_if_asr_failed=True,
            use_ocr_if_no_subtitle=False,
            prefer_ocr_over_asr_when_text_visible=False,
            asr_vad_filter=True,
            review_subtitles_before_render=False,
            auto_render_after_translation=True,
            add_overlay=False,
            overlay_mode="none",
            subtitle_cover_enabled=True,
            add_bgm=True,
            keep_original_audio=True,
            original_audio_volume=0.9,
            bgm_volume=0.12,
        ),
    ),
    DouyinReupPreset(
        id=DouyinReupPresetMode.clean_subtitle_only,
        name="Clean Subtitle Only",
        description="Translate and burn clean subtitles without overlay or background music.",
        recommended_for=["minimal edit", "subtitle QA", "keep original look"],
        not_recommended_for=["sale-style recut", "heavy branding"],
        ui_badge="Clean",
        settings=_settings(
            DouyinReupPresetMode.clean_subtitle_only,
            "Clean Subtitle Only",
            subtitle_source_priority=["sidecar_srt", "embedded_subtitle", "asr", "ocr_hardsub"],
            use_sidecar_srt=True,
            use_embedded_subtitle=True,
            use_asr_if_no_subtitle=True,
            use_ocr_if_asr_failed=True,
            use_ocr_if_no_subtitle=True,
            review_subtitles_before_render=True,
            auto_render_after_translation=False,
            add_overlay=False,
            overlay_mode="none",
            subtitle_cover_enabled=True,
            add_bgm=False,
            burn_subtitle=True,
            keep_original_audio=True,
            original_audio_volume=1.0,
        ),
    ),
    DouyinReupPreset(
        id=DouyinReupPresetMode.music_recut,
        name="Music Recut",
        description="Auto-render with stronger BGM mix while keeping original audio lower.",
        recommended_for=["music montage", "product b-roll", "short sale clips"],
        not_recommended_for=["dialogue-heavy videos", "subtitle QA pass"],
        ui_badge="BGM",
        settings=_settings(
            DouyinReupPresetMode.music_recut,
            "Music Recut",
            subtitle_source_priority=["sidecar_srt", "embedded_subtitle", "ocr_hardsub", "asr"],
            use_sidecar_srt=True,
            use_embedded_subtitle=True,
            use_asr_if_no_subtitle=True,
            use_ocr_if_asr_failed=True,
            use_ocr_if_no_subtitle=True,
            prefer_ocr_over_asr_when_text_visible=True,
            ocr_sample_fps=2.0,
            ocr_min_confidence=0.35,
            review_subtitles_before_render=False,
            auto_render_after_translation=True,
            add_overlay=False,
            overlay_mode="none",
            subtitle_cover_enabled=True,
            add_bgm=True,
            keep_original_audio=True,
            original_audio_volume=0.55,
            bgm_volume=0.28,
            enable_subtitle_rewrite_suggestions=True,
            default_rewrite_style="casual_tiktok",
        ),
    ),
    DouyinReupPreset(
        id=DouyinReupPresetMode.silent_chill_immersive,
        name="Không thoại - Chill immersive",
        description="Tạo caption tiếng Việt ngắn theo cảnh/OCR cho video không thoại, giữ âm thanh gốc và thêm BGM nhẹ.",
        recommended_for=[
            "video mở hộp không thoại",
            "đồ gia dụng chỉ có tiếng thao tác",
            "video immersive có nhạc nhẹ",
        ],
        not_recommended_for=["video có lời thoại rõ", "video cần dịch thoại đầy đủ"],
        ui_badge="Silent",
        settings=_settings(
            DouyinReupPresetMode.silent_chill_immersive,
            "Không thoại - Chill immersive",
            enable_silent_immersive_mode=True,
            silent_mode_detection=True,
            silent_mode_strategy="chill_immersive",
            detect_speech_presence=True,
            use_visual_segments_for_silent_video=True,
            ocr_sample_fps=0.5,
            generate_visual_captions=False,
            generate_voiceover_for_silent_video=False,
            silent_review_before_render=True,
            review_subtitles_before_render=False,
            auto_render_after_translation=True,
            keep_immersive_original_audio=True,
            immersive_original_audio_volume=0.75,
            add_bgm_for_silent_video=True,
            immersive_bgm_volume=0.16,
            keep_original_audio=True,
            original_audio_volume=0.75,
            add_bgm=True,
            bgm_volume=0.16,
            visual_style_preset_id="transparent_caption_box",
            add_overlay=True,
            subtitle_cover_enabled=False,
            burn_subtitle=True,
        ),
    ),
    DouyinReupPreset(
        id=DouyinReupPresetMode.silent_product_voiceover,
        name="Không thoại - Tạo voice review Việt",
        description="Tạo caption và voiceover tiếng Việt mới cho video visual rõ, phù hợp khi có thông tin sản phẩm.",
        recommended_for=[
            "video không thoại có visual rõ",
            "review tiếng Việt",
            "có tên sản phẩm hoặc ngành hàng",
        ],
        not_recommended_for=["video có thoại cần dịch nguyên văn", "video visual quá rời rạc"],
        ui_badge="Voice",
        settings=_settings(
            DouyinReupPresetMode.silent_product_voiceover,
            "Không thoại - Tạo voice review Việt",
            enable_silent_immersive_mode=True,
            silent_mode_detection=True,
            silent_mode_strategy="product_review_voiceover",
            detect_speech_presence=True,
            use_visual_segments_for_silent_video=True,
            ocr_sample_fps=0.5,
            generate_visual_captions=True,
            generate_voiceover_for_silent_video=True,
            silent_voiceover_provider="edge_tts",
            silent_voiceover_voice="vi-VN-HoaiMyNeural",
            silent_review_before_render=True,
            review_subtitles_before_render=True,
            auto_render_after_translation=False,
            keep_immersive_original_audio=True,
            immersive_original_audio_volume=0.45,
            add_bgm_for_silent_video=True,
            immersive_bgm_volume=0.12,
            keep_original_audio=True,
            original_audio_volume=0.45,
            add_bgm=True,
            bgm_volume=0.12,
            visual_style_preset_id="clean_review_light",
            add_overlay=True,
            subtitle_cover_enabled=False,
            burn_subtitle=True,
        ),
    ),
    DouyinReupPreset(
        id=DouyinReupPresetMode.silent_sales_recut,
        name="Không thoại - Recut bán hàng nhanh",
        description="Caption nhanh, rõ ý hơn cho video đồ tiện ích hoặc gia dụng không thoại.",
        recommended_for=[
            "đồ gia dụng",
            "đồ tiện ích",
            "video ngắn bán hàng",
        ],
        not_recommended_for=["video cần giữ mood tối giản", "video có thoại dài"],
        ui_badge="Sales",
        settings=_settings(
            DouyinReupPresetMode.silent_sales_recut,
            "Không thoại - Recut bán hàng nhanh",
            enable_silent_immersive_mode=True,
            silent_mode_detection=True,
            silent_mode_strategy="sales_recut",
            detect_speech_presence=True,
            use_visual_segments_for_silent_video=True,
            ocr_sample_fps=0.5,
            generate_visual_captions=True,
            generate_voiceover_for_silent_video=False,
            silent_review_before_render=True,
            review_subtitles_before_render=True,
            auto_render_after_translation=False,
            keep_immersive_original_audio=True,
            immersive_original_audio_volume=0.55,
            add_bgm_for_silent_video=True,
            immersive_bgm_volume=0.22,
            keep_original_audio=True,
            original_audio_volume=0.55,
            add_bgm=True,
            review_subtitles_before_render=False,
            auto_render_after_translation=True,
            keep_immersive_original_audio=True,
            immersive_original_audio_volume=0.75,
            add_bgm_for_silent_video=True,
            immersive_bgm_volume=0.16,
            keep_original_audio=True,
            original_audio_volume=0.75,
            add_bgm=True,
            bgm_volume=0.16,
            visual_style_preset_id="transparent_caption_box",
            add_overlay=True,
            subtitle_cover_enabled=False,
            burn_subtitle=True,
        ),
    ),
    DouyinReupPreset(
        id=DouyinReupPresetMode.silent_product_voiceover,
        name="Không thoại - Tạo voice review Việt",
        description="Tạo caption và voiceover tiếng Việt mới cho video visual rõ, phù hợp khi có thông tin sản phẩm.",
        recommended_for=[
            "video không thoại có visual rõ",
            "review tiếng Việt",
            "có tên sản phẩm hoặc ngành hàng",
        ],
        not_recommended_for=["video có thoại cần dịch nguyên văn", "video visual quá rời rạc"],
        ui_badge="Voice",
        settings=_settings(
            DouyinReupPresetMode.silent_product_voiceover,
            "Không thoại - Tạo voice review Việt",
            enable_silent_immersive_mode=True,
            silent_mode_detection=True,
            silent_mode_strategy="product_review_voiceover",
            detect_speech_presence=True,
            use_visual_segments_for_silent_video=True,
            ocr_sample_fps=0.5,
            generate_visual_captions=True,
            generate_voiceover_for_silent_video=True,
            silent_voiceover_provider="edge_tts",
            silent_voiceover_voice="vi-VN-HoaiMyNeural",
            silent_review_before_render=True,
            review_subtitles_before_render=True,
            auto_render_after_translation=False,
            keep_immersive_original_audio=True,
            immersive_original_audio_volume=0.45,
            add_bgm_for_silent_video=True,
            immersive_bgm_volume=0.12,
            keep_original_audio=True,
            original_audio_volume=0.45,
            add_bgm=True,
            bgm_volume=0.12,
            visual_style_preset_id="clean_review_light",
            add_overlay=True,
            subtitle_cover_enabled=False,
            burn_subtitle=True,
        ),
    ),
    DouyinReupPreset(
        id=DouyinReupPresetMode.silent_sales_recut,
        name="Không thoại - Recut bán hàng nhanh",
        description="Caption nhanh, rõ ý hơn cho video đồ tiện ích hoặc gia dụng không thoại.",
        recommended_for=[
            "đồ gia dụng",
            "đồ tiện ích",
            "video ngắn bán hàng",
        ],
        not_recommended_for=["video cần giữ mood tối giản", "video có thoại dài"],
        ui_badge="Sales",
        settings=_settings(
            DouyinReupPresetMode.silent_sales_recut,
            "Không thoại - Recut bán hàng nhanh",
            enable_silent_immersive_mode=True,
            silent_mode_detection=True,
            silent_mode_strategy="sales_recut",
            detect_speech_presence=True,
            use_visual_segments_for_silent_video=True,
            ocr_sample_fps=0.5,
            generate_visual_captions=True,
            generate_voiceover_for_silent_video=False,
            silent_review_before_render=True,
            review_subtitles_before_render=True,
            auto_render_after_translation=False,
            keep_immersive_original_audio=True,
            immersive_original_audio_volume=0.55,
            add_bgm_for_silent_video=True,
            immersive_bgm_volume=0.22,
            keep_original_audio=True,
            original_audio_volume=0.55,
            add_bgm=True,
            bgm_volume=0.22,
            visual_style_preset_id="cute_pastel_shop",
            add_overlay=True,
            subtitle_cover_enabled=False,
            burn_subtitle=True,
        ),
    ),
    DouyinReupPreset(
        id=DouyinReupPresetMode.long_movie_sub_only,
        name="Phim/Vlog - Chỉ chèn Sub Việt",
        description="Chỉ dịch thuật và chèn phụ đề tiếng Việt, giữ nguyên toàn bộ âm thanh gốc của phim/vlog.",
        recommended_for=["Phim chiếu rạp", "Vlog nước ngoài", "Phim tài liệu cần giữ âm thanh gốc"],
        not_recommended_for=["Video ngắn bán hàng"],
        ui_badge="Việt Sub",
        settings=_settings(
            DouyinReupPresetMode.long_movie_sub_only,
            "Phim/Vlog - Chỉ chèn Sub Việt",
            long_video_mode="viet_sub",
            review_subtitles_before_render=True,
            auto_render_after_translation=False,
            add_overlay=False,
            subtitle_cover_enabled=True,
            add_bgm=False,
            burn_subtitle=True,
            keep_original_audio=True,
            original_audio_volume=1.0,
            isolate_ambient_sound=False,
        ),
    ),
    DouyinReupPreset(
        id=DouyinReupPresetMode.long_movie_dubbing,
        name="Phim/Vlog - Thuyết minh Việt",
        description="Dịch phụ đề và lồng tiếng thuyết minh tiếng Việt, tự động tách thoại gốc nhưng giữ tiếng động môi trường.",
        recommended_for=["Phim thuyết minh", "Review phim ngắn", "Vlog thuyết minh"],
        not_recommended_for=["Video ca nhạc"],
        ui_badge="Thuyết Minh",
        settings=_settings(
            DouyinReupPresetMode.long_movie_dubbing,
            "Phim/Vlog - Thuyết minh Việt",
            long_video_mode="dubbing",
            review_subtitles_before_render=True,
            auto_render_after_translation=False,
            add_overlay=False,
            subtitle_cover_enabled=True,
            add_bgm=False,
            burn_subtitle=True,
            keep_original_audio=True,
            original_audio_volume=0.9,
            isolate_ambient_sound=True,
            generate_voiceover_for_silent_video=True,
            silent_voiceover_provider="edge_tts",
            silent_voiceover_voice="vi-VN-HoaiMyNeural",
        ),
    ),
]
