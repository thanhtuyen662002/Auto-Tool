...
def test_build_srt_translation_prompt_requires_json_response():
    prompt = build_srt_translation_prompt(SOURCE_SRT)
...
def test_subtitle_translator_raises_when_gemini_fails_by_default(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("AUTO_TOOL_ALLOW_TRANSLATION_FALLBACK", raising=False)

    with pytest.raises(RuntimeError, match="Gemini"):
        SubtitleTranslator().translate_srt(str(source), str(output), api_keys=["fake"])

    assert not output.exists()


def test_subtitle_translator_falls_back_to_source_when_explicitly_enabled(tmp_path, monkeypatch):
    source = tmp_path / "source.srt"
    output = tmp_path / "vi.srt"
    source.write_text(SOURCE_SRT, encoding="utf-8")
    ...
    monkeypatch.setattr(SubtitleTranslator, "_translate_blocks_with_gemini", fail_translate)
    monkeypatch.setenv("AUTO_TOOL_ALLOW_TRANSLATION_FALLBACK", "1")
