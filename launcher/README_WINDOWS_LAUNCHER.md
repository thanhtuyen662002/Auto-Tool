...
- Trang `Settings -> Dữ liệu`: cách an toàn hơn để xem dung lượng, tạo backup, restore và cleanup cache/log/temp trong app.
...
## Backup trước khi reset hoặc update

Trước khi reset cache, update app hoặc build EXE mới, nên mở `Settings -> Dữ liệu` và tạo backup mặc định. File backup nằm trong thư mục `backups/` và có manifest đi kèm để inspect trước khi restore.

## Khi app bị tắt giữa batch

Mở lại `start_auto_tool_studio.bat`, sau đó vào `Recovery Center` hoặc đường dẫn `/recovery`. App sẽ hiển thị các job bị gián đoạn, cho phép reconcile output và resume an toàn. App không tự resume khi vừa mở lại.
