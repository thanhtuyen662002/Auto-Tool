  ...
  return (
    <>
      ...
      >
        <Settings2 size={16} />
        Advanced Settings
        {custom ? <GlassBadge variant="warning">Custom settings</GlassBadge> : null}
      </button>
      {open ? (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/55" role="dialog" aria-modal="true" aria-label="Advanced Settings">
          <button className="hidden flex-1 cursor-default md:block" type="button" aria-label="Đóng Advanced Settings" onClick={onClose} />
          <aside className="h-full w-full max-w-3xl overflow-auto border-l border-white/10 bg-slate-950/95 p-5 shadow-2xl backdrop-blur">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {['Subtitle', 'Nhận diện giọng nói', 'Nhận diện chữ', 'Audio', 'Output', 'Kiểm tra cuối', 'Gói xuất bản'].map((item) => (
