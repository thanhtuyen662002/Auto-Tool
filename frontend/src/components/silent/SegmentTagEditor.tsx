...
export default function SegmentTagEditor({ segment, vocabulary, disabled, onSave, onRegenerate }: Props) {
  ...
  const groups = useMemo(
    () => [
      { label: 'Ngành hàng (Industry)', values: vocabulary.industry },
      { label: 'Bối cảnh (Scene)', values: vocabulary.scene },
      { label: 'Hành động (Action)', values: vocabulary.action },
      { label: 'Trạng thái sản phẩm (Product stage)', values: vocabulary.product_stage },
      { label: 'Chất lượng (Quality)', values: vocabulary.quality },
    ],
    [vocabulary],
  );
  ...
  return (
    <div className="grid gap-3 border-t border-line pt-3">
      <div className="grid gap-3 sm:grid-cols-3">
        <TagSelect label="Ngành hàng chính" value={primaryIndustry} options={vocabulary.industry} onChange={setPrimaryIndustry} />
        <TagSelect label="Bối cảnh chính" value={primaryScene} options={vocabulary.scene} onChange={setPrimaryScene} allowEmpty />
        <TagSelect label="Hành động chính" value={primaryAction} options={vocabulary.action} onChange={setPrimaryAction} allowEmpty />
      </div>
  ...
  return (
    <label className="block">
      ...
      <select className="h-9 w-full rounded-md border border-line bg-white px-2 text-xs" value={value} onChange={(event) => onChange(event.target.value)}>
        {allowEmpty ? <option value="">Không</option> : null}
