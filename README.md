...
## Auto Tool Douyin Reup v1.0 Overview

Auto Tool Douyin Reup v1.0.0-rc1 is a local release candidate for processing Douyin-style videos that the user already has permission to use. The goal of this RC is stability, clear logs, reviewable subtitles, usable output videos, and a repeatable QA flow.

Supported v1.0 flow:

```txt
Choose local Douyin video folder
-> Choose preset
-> Scan videos
-> Detect subtitle source
-> ASR or OCR if needed
-> Translate Chinese subtitles to Vietnamese
-> Score subtitle quality
-> Review / edit / rewrite subtitle
-> Approve subtitle
-> Render video with Vietnamese subtitle + overlay + BGM
-> Run Final Output QA
-> Create Export Pack for TikTok/Reels/Shorts
```
...
1. Use preset `Safe Review`.
2. Test with 3-5 videos first.
3. Check subtitle review documents.
4. Render 1 approved video.
5. Review Final QA.
6. Run larger batches only after the small test is clean.
...
- `Safe Review`: default first-run preset. Detect/translate subtitles and stop for manual review before render.
- `Fast Auto`: translate and render automatically for known-good input.
- `OCR Priority`: prefer OCR for videos with visible Chinese hard-sub text.
- `Voice Priority`: prefer ASR for clear spoken Chinese.
- `Clean Subtitle Only`: no overlay and no BGM, focused on clean subtitle output.
- `Music Recut`: auto-render with stronger BGM mix for montage-style videos.
- `Không thoại - Chill immersive`: for videos with no clear speech, only music or operation sounds. Generates short Vietnamese visual captions from scene/OCR, keeps original audio, and adds light BGM.
...
```txt
Local video + original audio
-> Speech presence check
-> Visual segment analysis
-> OCR Chinese hard-sub if available
-> Vietnamese visual captions if OCR has no usable text
-> Optional Vietnamese voiceover
-> Existing overlay/subtitle style + BGM mixer
-> Final QA and export pack
```
...
- Visual caption generation is rule/template-based, not deep AI video understanding.
- If product context is empty, captions stay generic.
...
```txt
examples/douyin_reup_v1_rc/
docs/DOUYIN_REUP_V1_RC_QA_CHECKLIST.md
docs/DOUYIN_REUP_V1_RELEASE_BLOCKERS.md
docs/DOUYIN_REUP_V1_PERFORMANCE_BASELINE.md
RELEASE_NOTES.md
```

Put local test videos in:

```txt
sample_videos/douyin_reup_v1_rc/
```

Do not commit real videos to the repo.

## Douyin Reup Troubleshooting

- `FFmpeg not found`: install FFmpeg or open the packaged app so Auto Tool can check runtime dependencies.
- `ffprobe not found`: install full FFmpeg, not only `ffmpeg.exe`.
- `Source folder not found`: choose an existing local folder.
- `No video files found`: put supported files (`.mp4`, `.mov`, `.mkv`, `.webm`) in the selected folder.
- `faster-whisper not installed`: install backend requirements or run RC with `--mock-asr`.
- `PaddleOCR/EasyOCR not installed`: wait for auto-install or run RC with `--mock-ocr`.
- `Gemini translation failed`: check Gemini keys or run RC with `--mock-translation`.
- `Invalid SRT format`: verify block number, timestamp line, and subtitle text.
- `BGM folder empty`: choose a folder with supported audio files.
- `Render failed`: open the per-video `video_XXX_log.json` and check FFmpeg error.
- `Final QA failed`: inspect the per-video final QA JSON and output media.
- `Export pack creation failed`: make sure at least one rendered output exists.

## Douyin Reup Known Limitations

- Tool does not download Douyin videos.
- Tool does not remove watermarks or Chinese hard-sub text already burned into the video image.
...
- Automatic translation must be reviewed by the user before posting.
- Final QA is rule-based technical validation and does not replace watching the final video manually.
- Users must ensure they have rights to use the source videos, music, and other assets.
...
Douyin Reup là workflow local cho các video Douyin đã được người dùng tải sẵn vào máy. Tool không tải video từ Douyin, không bypass watermark và không xóa watermark. OCR hard-sub chỉ chạy local trên video người dùng đã có sẵn. Người dùng cần tự đảm bảo có quyền sử dụng video, subtitle và nhạc nền.
...
-> Nếu ASR fail hoặc không có subtitle thì fallback OCR hard-sub tiếng Trung
...
Mỗi Subtitle Review Document được chấm điểm rule-based ngay sau khi tạo. UI hiển thị average score, các dòng cần review, critical/warning, bộ lọc, điều hướng đến dòng bị flag và approval guard. Các rule bao gồm độ dài, số dòng, reading speed, duration, ký tự Trung còn sót, markdown/JSON leak, ký tự lạ, OCR/ASR confidence, source-target mismatch, lặp text và timestamp overlap/out-of-range.

Preset `safe_review` bật `auto_mark_low_quality_lines=true`; dòng có score thấp hoặc critical sẽ được đánh dấu `needs_fix`. Báo cáo được lưu tại `subtitle_quality_report.json`, và `douyin_reup_summary.json` có thêm tổng hợp `subtitle_quality`. Nút `Suggest shorter translation` tạo gợi ý cho từng dòng, không tự động rewrite toàn bộ subtitle.

Known limitations:

- Quality score là rule-based, không đảm bảo phát hiện mọi lỗi dịch.
- Tool không thể kết luận bản dịch đúng 100%; user vẫn nên kiểm tra các dòng bị flag trước khi render.
- OCR/ASR confidence thấp chỉ là tín hiệu cần review, không phải kết luận chắc chắn.
- Rewrite suggestion mặc định là local rule-based khi không cấu hình AI provider.

### Subtitle Auto Shortener and Safe Rewrite

Trong Subtitle Review, các dòng bị quality flag có nút `Suggest rewrite`. Tool tạo tối đa 3 gợi ý ngắn hơn, tính trước quality score/CPS, kiểm tra keyword, thương hiệu, số liệu, đơn vị, markdown/JSON, ký tự Trung và forbidden claims. User chọn `Apply` để cập nhật `edited_text`; quality report được refresh ngay sau đó.

Bulk rewrite cho phép chọn style, issue type và số dòng tối đa. `Auto apply only safe high-score suggestions` chỉ áp dụng khi suggestion không có safety warning, ngắn hơn bản gốc và có quality score từ 85% trở lên. Mặc định tính năng này tắt.
...
Rewrite data được lưu trong SQLite, line `rewrite_history` và `subtitle_rewrite_log.json`. `douyin_reup_summary.json` có thêm `subtitle_rewrite` với số suggestion đã tạo/áp dụng và quality improvement trung bình.

Known limitations:

- Rewrite suggestion không đảm bảo đúng 100%; user vẫn nên kiểm tra trước khi approve.
- AI có thể đưa ra gợi ý chưa hoàn hảo dù safety validator đã chặn các lỗi rõ ràng.
...
- Final QA là kiểm tra kỹ thuật rule-based, không thay thế việc xem lại video bằng mắt.
- Subtitle visibility checker chua OCR lai video final de xac nhan chu da burn 100%.
- Platform profiles là gợi ý kỹ thuật local, không đảm bảo luôn khớp chính sách mới nhất.
- Tool không tự đăng video hoặc đăng nhập nền tảng.
...
- OCR hard-sub được dùng sau sidecar/embedded/ASR, hoặc khi bật `prefer_ocr_over_asr_when_text_visible`. Provider mặc định là `easyocr`; có thể đổi sang `paddleocr` hoặc `mock_ocr` để test pipeline không cần model OCR.
- Khi mở app, Auto Tool tự kiểm tra và tải runtime cần thiết trong background: FFmpeg, Piper voice/model và OCR provider. OCR package được cài vào `%LOCALAPPDATA%\AutoTool\python_packages\pyXX` nếu chưa có trong environment.
- PaddleOCR cần `paddlepaddle` wheel phù hợp với Python đang chạy. Nếu paddlepaddle không hỗ trợ Python hiện tại, dùng EasyOCR mặc định.
- Có thể tắt auto-install bằng `AUTO_TOOL_AUTO_INSTALL=0`, hoặc chỉ tắt OCR auto-install bằng `AUTO_TOOL_AUTO_INSTALL_OCR=0`.
- Xem trạng thái runtime qua `GET /api/system/dependencies`; UI Douyin Reup cũng hiển thị trạng thái OCR runtime.
...
## Release Candidate Testing (v1.0.0-rc1)

Auto Tool Douyin Reup hiện đang ở trạng thái **Release Candidate v1.0.0-rc1**.
